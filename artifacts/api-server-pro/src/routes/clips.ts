...
router.post("/clips/upload", (req, res): void => {
  ...
  bb.on("field", (name: string, value: string) => {
    ...
    if (name === "format") format = value || "essay";
    if (name === "essayScript") essayScript = value;
  });
  ...
  bb.on("finish", () => {
    void (async () => {
      try {
        ...
        const outputFilename = `clip_${clip.id}_${Date.now()}.mp4`;
        dispatchClipJob(clip.id, null, startTime, endTime, headline, outputFilename, mode, frameStyle, savedFilePath, sourceChannel, captionsEnabled, outroEnabled, voiceoverEnabled, voiceoverHook, punchInEnabled, zoomMoments, format, essayScript);
      } catch (err) {
        if (savedFilePath) {
